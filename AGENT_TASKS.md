...
- [x] **Orphaned sidecar processes survive app quit / crash (CRITICAL)** — Fixed 2026-03-23: Multiple root causes identified and fixed:
  ...
  10. **Windows watchdog false-positive kills engine at startup (CRITICAL)** — Fixed 2026-03-25: On Windows, Tauri spawns the sidecar via an intermediate pipe/shim helper process (not the Tauri app itself). `os.getppid()` in Python returns the PID of this short-lived launcher shim, which exits almost immediately after spawning the child. The watchdog saw the shim die, thought the parent was gone, and killed the engine ~2 seconds into startup every time. Fix: Rust now passes `TAURI_APP_PID=<tauri_pid>` as an env var when spawning the sidecar. Python watchdog reads this on Windows and watches the Tauri PID directly instead of the shim's PPID. Falls back to `os.getppid()` if the env var is absent (standalone/dev mode). macOS/Linux are unaffected — they use `os.getppid()` unchanged. Files: `run.py`, `desktop/src-tauri/src/lib.rs`.
...
- [x] **Windows: GPU not detected / used; CPU-only binary shipped (CRITICAL)** — Fixed 2026-03-23: Four separate bugs found and fixed:
  1. **CPU-only binary shipped for Windows**: `download-llama-server.sh` fetched `llama-${ver}-bin-win-cpu-x64.zip` — the CPU-only build. Users with ANY GPU (NVIDIA/AMD/Intel) got zero acceleration. Fixed: now ships `llama-${ver}-bin-win-vulkan-x64.zip` — the Vulkan build works for all modern GPUs via a single binary and falls back to CPU for CPU-only machines.
  2. **No Vulkan GPU detection**: `hardware.rs` only ran `nvidia-smi` to detect GPUs. If nvidia-smi wasn't on PATH (common), or user had AMD/Intel, `supports_cuda = false` and `gpu_vram_mb = None` even on a high-end gaming PC. Fixed: added multi-tier detection: `nvidia-smi` → `vulkaninfo` → `wmic` (Windows) → `/sys/class/drm` (Linux). Added `supports_vulkan` and `gpu_name` fields to `HardwareProfile`.
  3. **Model selector ignored Vulkan GPUs**: `select_tier()` only checked `supports_cuda`, not `supports_vulkan`. Vulkan GPU owners got CPU inference path and `gpu_layers = 0`. Fixed: both CUDA and Vulkan handled in same GPU path. Also: VRAM < 2GB case was a silent fallthrough; now returns `LowAlt` tier with partial offload layers.
  4. **`LowAlt` and `HighAlt` tiers unreachable**: `select_tier()` never returned `LowAlt` (Phi-4-mini) or `HighAlt` (Mistral-24B). Fixed: `LowAlt` returned for <4GB RAM CPU machines; `HighAlt` returned for 16GB+ VRAM GPU machines.
  Files: `hardware.rs`, `model_selector.rs`, `server.rs`, `download-llama-server.sh`, `types.ts`, `LocalModels.tsx`.

- [x] **Windows: local models crash with "llama_params_fit: failed to load model"** — Fixed 2026-03-23 (updated): Root cause analysis identified three compounding issues specific to Windows:
  1. **`llama_params_fit` probe fails on Windows** — `llama_params_fit` is a newer llama.cpp feature (b7300+) that auto-fits GPU layers by loading the model internally before the real load. On Windows it triggers an internal load failure that bubbles up as the misleading "failed to load model" error even when the file is perfectly valid. Fix: pass `-fit off` on Windows to skip the probe and use the caller-supplied `-ngl` directly (already correctly computed by `detect_llm_hardware`).
  2. **`-fa on` not supported by CPU-only Windows binary** — The flash attention flag (`-fa on`) passed to `llama_params_fit`'s internal probe causes a failure in the CPU-only Windows llama-server binary. macOS Metal and Linux CUDA builds support it; the CPU Windows build does not. Fix: `-fa on` is now only passed on non-Windows builds.
