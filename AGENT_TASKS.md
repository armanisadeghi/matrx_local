...
- [ ] **TTS settings parity gap** — 4 TTS settings (`ttsAutoDownloadModel`, `ttsDefaultSpeed`, `ttsDefaultVoice`, `ttsFavoriteVoices`) exist in TS `AppSettings` but not in Python `DEFAULT_SETTINGS`. Also not assigned to any section in `SECTION_KEYS`. Tests `test_ts_keys_not_in_python`, `test_all_app_settings_keys_are_in_a_section`, `test_each_sidebar_route_has_page_file` (TTS page file name mismatch) are failing.
