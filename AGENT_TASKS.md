...
- [x] **Windows installer file-lock on upgrade** — NSIS installer failed with "Error opening file for writing: aimatrx-engine.exe" even when no process was visible in Task Manager. Root cause: PyInstaller `--onefile` binaries extract to a random `_MEIxxxxxx` temp dir at launch; Windows Restart Manager registers those extracted files as "in use" by the installer package, and those registrations survive process exit. Fixed by: (1) creating `aimatrx-engine-x86_64-pc-windows-msvc.spec` with `runtime_tmpdir='%LOCALAPPDATA%\\AI Matrx\\engine-runtime'` (fixed extraction dir instead of random temp), (2) switching `tauri.conf.json` from WiX to NSIS, and (3) adding `src-tauri/nsis/installer-hooks.nsh` with `NSIS_HOOK_PREINSTALL` that kills any surviving processes AND deletes the fixed extraction dir before file copy begins, clearing all stale Restart Manager registrations. macOS/Linux are not affected (no Restart Manager, `/tmp` is cleaned by the OS).
