...
- [ ] **Wake word: replace "hey matrx" with trained phonemes** — "matrx" is not in whisper's vocabulary. Current: substring match on "matrix" or "matrx". Future: sherpa-onnx KWS with BPE tokenization handles this correctly.
...
- [x] **Agent picker (prompts parity)** — Updated 2026-03-18: `AgentPicker.tsx` aligned with matrx-admin prompts UX — weighted **multi-token** search (name/description/category/tags/id/variables), **inclusion** chips for category & tags with **No category** / **No tags** (`NONE_SENTINEL`), favorites **All / only / exclude**, **Favorites first** pin (when showing all), sort adds **Category (A–Z)**. Public **`prompt_builtins`** are labeled **Catalog** with always-visible **Show** pills: All · Mine · Catalog · Shared. Reference: `matrx-admin/features/prompts/.../PromptsGrid.tsx`, `DesktopFilterPanel.tsx`, `DesktopSearchBar.tsx`.
