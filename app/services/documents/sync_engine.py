...
class SyncEngine:
    ...
    async def push_note(
        ...
        """Create or update a note locally, then sync to Supabase.

        Local file write always happens first and is **never** blocked by
        Supabase availability.  Pass ``is_new_note=True`` when the ``note_id``
        was just generated by the caller — this skips the existence check and
        issues an atomic upsert instead of get→create, which is faster and
        correct.  For existing notes the old content is fetched first so a
        version snapshot can be saved before overwriting.
        """
        if not self._user_id:
            raise RuntimeError("Sync engine not configured — call configure() first")

        # ── 1. Local file — always, independent of Supabase ──────────────────
        file_path = self.fm.write_note(folder_name, label, content)
        c_hash = content_hash(content)

        # Echo-suppression: update immediately so the file-watcher ignores this write
        self._last_push_hashes[file_path] = c_hash

        # Persist local sync state right away
        state = self.fm.load_sync_state()
        state["note_hashes"][file_path] = c_hash
        self.fm.save_sync_state(state)

        # ── 2. Fallback result (returned when Supabase is unavailable) ────────
        result: dict[str, Any] = {
            "id": note_id,
            "label": label,
            "content": content,
        ...
        # ── 3. Supabase sync (best-effort; local file already safe) ──────────
        if self.sb.available:
            try:
                if is_new_note:
                    # Fresh UUID — no existence check needed; upsert is atomic
                    # and idempotent on retries.
                    ...
                    # Existing note: fetch current content for version snapshot.
                    ...
                    if existing:
                        ...
                        # Note missing from Supabase (created offline); push it now.
                        result = await self.sb.upsert_note(
                            note_id=note_id,
                            user_id=self._user_id,
                            label=label,
        ...
        # ── 4. Mirror to mapped directories ──────────────────────────────────
    ...
    async def resolve_conflict(
        ...
        # remote.md stays on disk as reference; we re-fetch from Supabase below.
