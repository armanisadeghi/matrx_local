...
class AudioFile(AudioSource):
    ...
    class AudioFileStream(object):
        def __init__(self, audio_reader, little_endian, samples_24_bit_pretending_to_be_32_bit):
            ...
            self.little_endian = (
                little_endian  # whether the audio data is little-endian (when working with big-endian things, we'll have to convert it to little-endian before we process it)
            )
...
class Recognizer(AudioSource):
    def __init__(self):
        ...
        self.phrase_threshold = (
            0.3  # minimum seconds of speaking audio before we consider the speaking audio a phrase - values below this are ignored (for filtering out clicks and pops)
        )
    ...
    def recognize_sphinx(
        ...
        assert keyword_entries is None or all(
            isinstance(keyword, (type(""), type(""))) and 0 <= sensitivity <= 1 for keyword, sensitivity in keyword_entries
    ...
    def recognize_google_cloud(
        ...
        assert preferred_phrases is None or all(
            isinstance(preferred_phrases, (type(""), type(""))) for preferred_phrases in preferred_phrases
