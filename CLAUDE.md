...
6. **PyInstaller hidden imports must stay in sync with pyproject.toml** -- When a FastAPI route uses `Form`, `File`, or `UploadFile` (which require `python-multipart`), or any other package that PyInstaller cannot auto-discover, it must be added to `hiddenimports` in all four `.spec` files (under `specs/`: `aimatrx-engine-aarch64-apple-darwin.spec`, `aimatrx-engine-x86_64-apple-darwin.spec`, `aimatrx-engine-x86_64-unknown-linux-gnu.spec`, `aimatrx-engine-x86_64-pc-windows-msvc.spec`) AND in the `build_with_flags()` fallback in `scripts/build-sidecar.sh`. Use the Python import name (`python_multipart`, not `python-multipart`). Omitting this causes silent runtime failures in the compiled sidecar that don't occur in dev mode.
